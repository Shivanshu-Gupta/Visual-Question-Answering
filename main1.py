...
def log(msg, stdout=True, file=None):
    if file is not None:
        print(msg, file)
    if stdout:
        print(msg)
...
def printHeaders(config):
    if 'trainTimeLogs' in config['stats']:
        time_log_hdrs = [
            'time', 'epoch', 'nbatches',
            'nsentences', 'nwords',
            'top1', 'top5', 'loss',
            'trainingTime', 'dataLoadingTime',
            'lr', 'whichModel']
        with open(config['stats']['trainTimeLogs'], 'a+') as fh:
            print(','.join(time_log_hdrs), file=fh)
    acc_log_hdrs = [
        'time', 'epoch', 'nbatches',
        'nsentences', 'nwords',
        'top1', 'top5', 'loss',
        'trainingTime', 'dataLoadingTime',
        'lr', 'whichSet', 'whichModel']
    with open(config['stats']['accuracyLogs'], 'a+') as fh:
        log(','.join(acc_log_hdrs), file=fh)
...
def accuracy(answer_scores, labels, topk=(1, ), dataloader=None, writeToFile=False, fh=None):
    ...
    if dataloader is not None and fh is not None:
        print('\n'.join(['{0}, {1}'.format(dataloader.dataset.itoa[pred[0][x]],
                                           dataloader.dataset.itoa[labels[x]]) for x in range(batchSize)]), file=fh)
    return res


def save_image_features(config, dataloader, net):
    net.eval()
    asyncVar = 'pin_memory' in config['data']['loader_params'] and config['data']['loader_params']['pin_memory']
    ...
    for i, (inputs, labels, images, image_ids) in uc.customVQADataBatcher(dataloader, config['data']['custom_batch_size']):
        ...
        inputs, labels, images = Variable(inputs, volatile=True), Variable(
            labels, volatile=True), Variable(images, volatile=True)
...
def validate(config, dataloader, net, criterion, optimizer, epoch, whichSet, whichModel):
    ...
    # for i, (inputs, labels) in enumerate(dataloader, 0):
        ...
        if writeToFile:
            prec1, prec5 = accuracy(output.data, labels.data, topk=(
                1, 5), dataloader=dataloader, writeToFile=writeToFile, fh=ofh)
        else:
    ...
    with open(config['stats']['accuracyLogs'], 'a+') as fh:
        ...
        log("{0}, {1}, {2}, {3}, {4}, {5:.3f}, {6:.3f}, {7:.3f}, {8:.2f}, {9:.2f}, {10:.5f}, {11}, {12}".format(
            dtstr, epoch, batchTime.count,
            nsentences, top1.count,
            top1.avg, top5.avg, losses.avg,
            dataLoadingTime.sum, batchTime.sum,
            lr, whichSet, whichModel), file=fh)
...
def train(config, trainloader, net, criterion, optimizer, epoch):
    ...
    # for i, (inputs, labels) in uc.customSentenceBatcher(trainloader, config['data']['custom_batch_size']):
        ...
        inputs, labels, images = Variable(
            inputs), Variable(labels), Variable(images)
        if torch.cuda.is_available():
            inputs, labels = inputs.cuda(), labels.cuda(async=asyncVar)
            images = images.cuda()
        ...
        dtstr = str(dt.now())
        if 'verbose' in config and config['verbose'] and i % 100 == 0:
            print("{0}, {1}, {2}, {3}, {4}, {5:.3f}, {6:.3f}, {7:.3f}, {8}, {9}".format(
                dtstr, epoch, trainingTime.count,
                nsentences, top1.count,
                top1.avg, top5.avg, losses.avg,
                trainingTime.sum, config['model_name']))
    ...
    with open(config['stats']['trainTimeLogs'], 'a+') as fh:
        log("{0}, {1}, {2}, {3}, {4}, {5:.3f}, {6:.3f}, {7:.3f}, {8:.2f}, {9:.2f}, {10:.5f}, {11}".format(
            dtstr, epoch, trainingTime.count,
            nsentences, top1.count,
            top1.avg, top5.avg, losses.avg,
            trainingTime.sum, dataLoadingTime.sum,
            lr, config['model_name']), file=fh)
...
def saveCheckpoint(state, isBest, config):
    ...
    if isBest:
        print("isBest True. Epoch: {0}, bestError: {1}".format(
            state['epoch'], state['bestError']))
        shutil.copyfile(config['checkpoints']['path'],
                        config['checkpoints']['best_model_path'])
...
def enhance_config(config):
    ...
    config['data']['features_dir'] = os.path.join(
        config['data']['path'], config['data']['features_dir'])
...
def main(config, mode, onlyTest=False):
    ...
    if(config['optim']['class'].lower() == 'sgd'):
        optimizer = optim.SGD(filter(
            lambda p: p.requires_grad, net.parameters()), **config['optim']['params'])
    else:
    ...
    if ((config['training']['start_from_checkpoint']) or (mode == 'test')):
        if os.path.exists(pathForTrainedModel):
            ...
            print("=> no checkpoint found at '{0}'".format(
                pathForTrainedModel))
            if mode == 'test':
                print(
                    " = > Error: could not find any model at the given path. Quitting...")
                return
    ...
    if mode == 'test':
        epoch = startEpoch - 1
        testLoss, testPrec1 = validate(
            config, testloader, net, criterion, optimizer, epoch, 'testSet', config['model_name'])
        if onlyTest:
            return
        validLoss, validPrec1 = validate(
            config, validationloader, net, criterion, optimizer, epoch, 'validationSet', config['model_name'])
        trainLoss, trainPrec1 = validate(
            config, trainloader, net, criterion, optimizer, epoch, 'trainSet', config['model_name'])
        return

    #
    # Tracer()
    # Pdb().set_trace()
    scheduler = uc.CustomReduceLROnPlateau(
    ...
    for epoch in range(startEpoch, config['training']['no_of_epochs']):
        ...
        validLoss1, validPrec1 = validate(
            config, validationloader, net, criterion, optimizer, epoch, 'validationSet', config['model_name'])
        trainLoss1, trainPrec1 = validate(
            config, trainloader, net, criterion, optimizer, epoch, 'trainSet', config['model_name'])
